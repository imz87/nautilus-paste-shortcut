...
That coupling makes automated testing harder because importing the current module requires `gi` and Nautilus bindings. The follow-up test task in `005-add-unit-tests-for-core-logic.md` depends on this small refactor that preserves current behavior while isolating the non-desktop logic behind a normal Python import boundary.
